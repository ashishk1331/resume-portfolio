...
Next day I went out and touched some grass. It made realize that we do not need a complex LLM recognition here. If I convert the characters to ascii (or specifically unicode here) we can segregate the sentences captured in the `texts`. Also, we already know that each `texts` property will contain text "duolingo" at the last index and might contain text "11" from the junior image parsing. Hence, this is actually a simple confing problem nothing related to complex LLM text summarization problem.
...
And for detecting the jp phrases from the english ones, we can use unicode comparision. Claude gave me the exact range and conditions.

```py
'\u3040' <= c <= '\u30ff' or '\u4e00' <= c <= '\u9fff'
```

These two conditions helped to sanitize the entire data. No ollama. No LFM. Also, I've attached the complete file including all 776 examples.
