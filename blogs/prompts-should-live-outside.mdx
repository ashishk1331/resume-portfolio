...
I strongly believe that values—here, the prompts being simple text—should exist outside the logic sphere. Just as you place images and other static assets in the public folder, separate from your code files, prompts should also be placed outside the coding files.

Building on this notion, I would like to present few ways of placing your prompts and context files in ways separate them from coding files.
...
For this section, I would like to thank [goober css](https://goober.rocks/) for introducing me to tagged functions in template strings. Also, credit to the inventor of the `clsx` function, used by shadcn UI components back in the day, when it was a one-liner: `classNames.filter(Boolean).join(' ')`. These two sparked a connection and prompted me to write a small tagged function that can be used with template literals and later return a function.

The returned function runs a regex to identify escaped entities, which are then substituted with values. Hence, I created the `pm` function (acronym for prompt-maker). The best part of `pm` is that it returns a function that can be reused multiple times to render the template with different sets of values.
...
The `pm` function still depends on the JavaScript runtime for the prompt files to be created, which makes it language-dependent. But what if I decided to create a dedicated Go server for handling API requests and logging? All the prompt files would still be needing JavaScript environment.
...
You can also use full-fledged templating engine to be on the safer side. Or anything in-between that satisfy your curiosity and requirements.

## Others on shelf options

The closest available option is the prompt templates on the [Vertex AI dashboard](https://cloud.google.com/vertex-ai/generative-ai/docs/learn/prompts/prompt-templates). However, the website lists several limitations, and they can’t be used with system prompts. The implementation process is stricter, and I’ve only tested them on the dashboard.
...
Most solutions have coupled the behavioral logic with the data itself. Think about it—do SQL tables know what query language they use? Or does HTML code know what a window represents for any browser?
...
Plus, I’m still exploring the prompting space and tend to jump into coding faster than doing my research. If you find the perfect solution to this problem, feel free to point it out to me. 🙇